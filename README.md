...
로그아웃시에는 SSE 연결 정보가 삭제되며, 캐시 스토리지에 저장된 SSE 연결 정보가 삭제됩니다.

이 프로젝트를 하면서 처음 설정한 목표에 도달하고, 고민할 점을 해소한 것 같습니다.

사실 배포한 서버에서 Spring Boot App을 3개 돌려서 확인해보려고 실행을 해보았으나, 통신이 너무 느린탓에 2개만 돌려서 테스트해봤습니다. 그래서 3개 이상은 로컬에서 테스트 해보려고합니다.
